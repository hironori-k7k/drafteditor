...
// ユーザー操作系
const ope = {
	theadRowLenSelect: document.getElementById('theadRowLenSelect'),
	setScopeCheck: document.getElementById('setScopeCheck'), // thタグへのscope属性設定の有無
	removeSpanCheck: document.getElementById('removeSpanCheck'), // spanタグがあった場合に削除するか
	removeStrongCheck: document.getElementById('removeStrongCheck'), // strongタグがあった場合に削除するか
	insertBrCheck: document.getElementById('insertBrCheck'), // 改行を保持するか（spanタグの連続部分に<br>挿入）
	removeNewlineCheck: document.getElementById('removeNewlineCheck'), // 改行を無くして1行で出力
	scopeValueOf1st: Array.from(document.getElementsByName('scopeValueOf1st')), // 1つ目のthのscope属性値
	releaseReadonlyCheck: document.getElementById('releaseReadonlyCheck'),
	functions: () => {
		// scope用ラジオボタンの可視性設定
		// ope.scopeRadiosDisplay();
		// scope用ラジオボタンの可視性変更用のイベントリスナー起動
		// ope.scopeRadiosListener();
		// readonly属性設定用のイベントリスナー起動
		ope.releaseReadonly();
	},
	scopeRadiosDisplay: () => {
		const span = document.getElementById('scopeRadios');
		this.setScopeCheck.checked ? span.classList.remove('invalid') : span.classList.add('invalid');
	},
	scopeRadiosListener: () => {
		this.setScopeCheck.addEventListener('input', () => {
			ope.scopeRadiosDisplay();
		});
	},
	releaseReadonly: () => {
...
// 原稿全体に対する単純な置換であれば、String.prototypeにて
// ブロック選択的な置換であれば、ブロック選択用関数にcallbackで置換内容を与える

function main() {
	elem.source.value = sample;
	let draft = elem.source.value;
	draft = draft
	.setAlt2ImgBlocks()
	.setNewTab2extLinks()
